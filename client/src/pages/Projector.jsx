        ...
        {phase === "playing" && (
          <div className="afro-card kente-border mt-6">
            ...
            <ul className="space-y-4">
              {hints.map((h, i) => (
                <li key={i} className="chip p-4 rounded-xl text-xl animate-tribal">{h}</li>
              ))}
            </ul>
          </div>
        )}
          ...
          <div className="mt-10">
            ...
            <div className="podium">
              <div className="place">
                ...
                <div className="opacity-80">{finalRank.podium[1]?.corrects ?? 0} acertos</div>
              </div>

              <div className="place first neon-gold">
                <div className="medal">🥇</div>
                <div className="font-bold mt-1 text-xl">{finalRank.podium[0]?.name || "-"}</div>
                <div className="opacity-90 font-semibold">{finalRank.podium[0]?.corrects ?? 0} acertos</div>
              </div>

              <div className="place">
                <div className="medal">🥉</div>
                <div className="font-bold mt-1">{finalRank.podium[2]?.name || "-"}</div>
                <div className="opacity-80">{finalRank.podium[2]?.corrects ?? 0} acertos</div>
              </div>
            </div>
            ...
            <div className="mt-6 afro-card kente-border">
              ...
              <ol className="space-y-2">
                {finalRank.charStats?.length ? finalRank.charStats.map((c, i) => (
